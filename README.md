...
- Vite
  ...
  - 开发环境优化：文件更改自动重启 Vite 服务，引入开发工具
...
- http 请求，使用 axios
  ...
  - 请求扩展，将请求方法包装一层，接收自定义请求选项
    ...
    - 文件下载
    - 响应状态码处理
      - http 错误状态码：例如 200 返回，401 跳登录，其它错误码跳对应页面
      - api 数据状态码：例如 弹出提示信息、再次确认
...
│   │       ├── base.scss
│   │       ├── common.scss
│   │       ├── compatible.scss               兼容性处理，例如：新 css 特性
│   │       ├── index.scss
│   │       ├── loading.scss
│   │       ├── mixins
│   │       │   ├── compatible.scss           兼容性处理，同上
│   │       │   ├── index.scss
│   │       │   ├── one-px.scss               1px 处理
│   │       │   └── safe-area.scss            安全区域处理，例如：刘海屏、曲面屏、底部 touch 按钮等
│   │       ├── optimize.scss                 优化处理，例如：开启硬件（GPU）加速
│   │       ├── platforms                     平台处理
│   │       │   ├── android.scss
│   │       │   ├── common.scss
│   │       │   ├── index.scss
...
│   │   ├── auto-imports.d.ts
│   │   ├── components.d.ts
│   │   ├── global.d.ts                       全局声明文件，跨平台处理时，注入 window 属性
│   │   ├── pinia.d.ts
│   │   └── vite-env.d.ts
